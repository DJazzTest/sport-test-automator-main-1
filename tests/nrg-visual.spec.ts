...
test('Navigate to In Play, select live event, and verify event details on NRG', async () => {
  ...
  try {
    // 1. Go to home page
    await page.goto('https://nrg.bet/', { waitUntil: 'domcontentloaded' });
    await page.screenshot({ path: 'test-results/01-homepage-nrg.png', fullPage: true });
    ...
    try {
      ...
      if (allowAllBtn) {
        ...
        await page.screenshot({ path: 'test-results/02-cookie-dismissed-nrg.png', fullPage: true });
    ...
    for (const selector of inPlaySelectors) {
      try {
        ...
        await page.screenshot({ path: 'test-results/04-inplay-clicked-nrg.png', fullPage: true });
    ...
    if (!foundInPlay) {
      ...
      await page.screenshot({ path: 'test-results/inplay-header-error-nrg.png', fullPage: true });
    ...
    if (!foundAllSports) {
      ...
      await page.screenshot({ path: 'test-results/allsports-tab-error-nrg.png', fullPage: true });
    ...
    for (let i = 0; i < toTest; i++) {
      ...
      await page.screenshot({ path: `test-results/event-detail-nrg-${i + 1}.png`, fullPage: true });
      const widgetSelector = 'app-the-sport-widget-component .animated_widget';
      const widget = await page.$(widgetSelector);
      if (widget) {
        await page.screenshot({ path: `test-results/widget-found-event-nrg-${i + 1}.png`, fullPage: true });
    ...
    if (widgetCount < toTest) {
      await page.screenshot({ path: 'test-results/not-enough-widgets-found-nrg.png', fullPage: true });
