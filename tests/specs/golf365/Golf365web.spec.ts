...
const _golf365EmailFailures: string[] = [];

/** Run a check and log ✅ passed or ❌ failed per section; collects failures for email report. */
async function checkAndLog(label: string, fn: () => Promise<void>): Promise<void> {
  try {
    ...
    _golf365EmailFailures.push(`Functional: ${label}`);
    throw e;
  }
}
...
test.describe('Golf365 Tests', () => {
  ...
  test.afterAll(() => {
    if (_golf365EmailFailures.length > 0) {
      appendEmailReportFailures('Golf365', _golf365EmailFailures);
    }
  });
