...
async function checkBrokenImages(page: Page, sectionName: string, failures?: string[]): Promise<number> {
  ...
  for (let i = 0; i < Math.min(imgCount, 50); i++) {
    ...
    try {
      ...
      if (width === 0) {
        ...
        failures?.push(`Broken image: ${sectionName} | ${fullUrl}`);
        brokenCount++;
      }
    } catch {}
  }
...
// Helper to check for ads presence (scroll + poll for lazy-loaded slots)
async function checkAdsPresence(page: Page, sectionName: string): Promise<boolean> {
  const ad = await detectDisplayAds(page, { deadlineMs: 18_000 });
  console.log(`📢 ${sectionName} ads: containers=${ad.containers} iframes=${ad.iframeAds} visible=${ad.visibleBlocks}`);
  return ad.found;
}
...
test('Planet Rugby – comprehensive site testing', async ({ page }) => {
  const isFast = !!(process.env.CI || process.env.GITHUB_ACTIONS || process.env.PLAYWRIGHT_QUICK || process.env.PLAYWRIGHT_EMAIL_SUITE === '1');
  test.setTimeout(isFast ? 5 * 60_000 : 15 * 60_000);

  // Detect CI / quick environment to reduce test scope
  const isCI = isFast;
  ...
  if (homeBrokenImages > 0) {
    ...
    emailFailures.push(`Broken image: Home page has ${homeBrokenImages} broken image(s)`);
  } else {
  ...
  if (articleCount > 0) {
    ...
    if (articlesPassed === articlesTested && articlesTested > 0) {
      ...
      emailFailures.push(`Broken URL: Home page articles — ${articlesTested - articlesPassed}/${articlesTested} failed`);
    }
  } else {
    console.log(`❌ SUMMARY: No article links found on home page`);
    emailFailures.push(`Functional: No article links found on home page`);
  }
  ...
  if (newsArticleCount > 0) {
    ...
    if (newsArticlesPassed === newsArticlesTested && newsArticlesTested > 0) {
      ...
      emailFailures.push(`Broken URL: News articles — ${newsArticlesTested - newsArticlesPassed}/${newsArticlesTested} failed`);
    }
  } else {
    console.log(`❌ SUMMARY: No article links found on News page`);
    emailFailures.push(`Functional: No article links found on News page`);
  }
  ...
  if (totalMatchInfo > 0) {
    ...
    if (matchesPassed === matchesTested && matchesTested > 0) {
      ...
      emailFailures.push(`Functional: Live scores — ${matchesTested - matchesPassed}/${matchesTested} match pages failed`);
    }
  } else {
    console.log(`❌ SUMMARY: No Match Info links found on Live Scores page`);
    emailFailures.push(`Functional: No Match Info links found on Live Scores page`);
  }
  ...
  if (tablesTested > 0 && tablesPassed < tablesTested) {
    emailFailures.push(`Functional: Tables — ${tablesTested - tablesPassed}/${tablesTested} failed`);
  }
  ...
  if (nationalTeamsTested > 0 && nationalTeamsPassed < nationalTeamsTested) {
    emailFailures.push(`Broken URL: National Teams — ${nationalTeamsTested - nationalTeamsPassed}/${nationalTeamsTested} failed to load`);
  }
  ...
  if (await premiershipBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (premTeamsTested > 0 && premTeamsPassed < premTeamsTested) {
      emailFailures.push(`Broken URL: Premiership Teams — ${premTeamsTested - premTeamsPassed}/${premTeamsTested} failed to load`);
    }
  }
  ...
  if (await urcBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (urcTeamsTested > 0 && urcTeamsPassed < urcTeamsTested) {
      emailFailures.push(`Broken URL: URC Teams — ${urcTeamsTested - urcTeamsPassed}/${urcTeamsTested} failed to load`);
    }
  }
  ...
  if (await superRugbyBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (srTeamsTested > 0 && srTeamsPassed < srTeamsTested) {
      emailFailures.push(`Broken URL: Super Rugby Teams — ${srTeamsTested - srTeamsPassed}/${srTeamsTested} failed to load`);
    }
  }
  
  if (emailFailures.length > 0) {
    appendEmailReportFailures('PlanetRugby', emailFailures);
  }

  console.log('\n✅ Planet Rugby comprehensive tests completed!');

  if (emailFailures.length > 0) {
    throw new Error(`PlanetRugby: ${emailFailures.length} failure(s) detected`);
  }
});
