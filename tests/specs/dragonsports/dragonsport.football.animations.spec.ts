...
test('DragonBet – Football Animation Check', async ({ page, context }) => {
  ...
  if (fail > 0) {
    const failures = [`Football animations: ${fail}/${maxToTest} events failed animation check`];
    appendEmailReportFailures('DragonSports', failures);
    throw new Error(`DragonSports Football: ${fail} animation failure(s)`);
  }
});
