...
const maxArticles = isQuick ? 2 : 6;
const maxVideos = isQuick ? 2 : 6;
...
async function visitArticlesAndCheck(page: Page, hrefs: string[], context: string, failures?: string[]) {
  for (let i = 0; i < hrefs.length; i++) {
    ...
    if (!resp || !resp.ok()) {
      ...
      failures?.push(`Broken URL: ${context} – article failed (HTTP ${resp?.status() ?? 'N/A'})`);
      continue;
    }
    ...
    for (const inner of innerLinks) {
      ...
      if (!innerResp || !innerResp.ok()) {
        ...
        failures?.push(`Broken URL: ${context} in-article link ${innerUrl} (HTTP ${innerResp?.status() ?? 'N/A'})`);
      } else {
...
test('DragonSportsUK – site content, navigation, Live Centre, and media smoke checks', async ({ page }) => {
  test.setTimeout(isQuick ? 5 * 60_000 : 10 * 60_000);
  ...
  const homeArticles = await sampleArticleLinks(page, maxArticles);
  console.log(`📋 Home – testing ${homeArticles.length} article(s)`);
  await visitArticlesAndCheck(page, homeArticles, 'Home', emailFailures);
  ...
  const rugbyNewsArticles = await sampleArticleLinks(page, maxArticles);
  console.log(`📋 Welsh Rugby News – testing ${rugbyNewsArticles.length} article(s)`);
  await visitArticlesAndCheck(page, rugbyNewsArticles, 'Welsh Rugby News', emailFailures);

  if (isQuick) {
    console.log('⏩ Quick mode: skipping teams, players, horse racing, video, and live centre deep checks');
  ...
  for (const name of teamNames) {
    ...
    if (!resp || !resp.ok()) {
      ...
      emailFailures.push(`Broken URL: Rugby team page ${name} (HTTP ${resp?.status() ?? 'N/A'})`);
    } else {
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    if (!resp || !resp.ok()) {
      ...
      emailFailures.push(`Broken URL: Rugby player page ${name} (HTTP ${resp?.status() ?? 'N/A'})`);
    } else {
  ...
  await visitArticlesAndCheck(page, wfNewsArticles, 'Welsh Football News', emailFailures);
  ...
  for (const tName of wfTeams) {
    ...
    if (!resp || !resp.ok()) {
      ...
      emailFailures.push(`Broken URL: Football team page ${tName} (HTTP ${resp?.status() ?? 'N/A'})`);
    } else {
  ...
  for (let i = 0; i < wfPlayerCount; i++) {
    ...
    if (!resp || !resp.ok()) {
      ...
      emailFailures.push(`Broken URL: Football player page ${name} (HTTP ${resp?.status() ?? 'N/A'})`);
    } else {
  ...
  if (await hrNews.isVisible({ timeout: 4000 }).catch(() => false)) {
    ...
    await visitArticlesAndCheck(page, hrNewsArticles, 'Horse Racing News', emailFailures);
  }
  ...
  if (await osNews.isVisible({ timeout: 4000 }).catch(() => false)) {
    ...
    await visitArticlesAndCheck(page, osNewsArticles, 'Other Sports News', emailFailures);
  }
  ...
  if (await wssLink.isVisible({ timeout: 4000 }).catch(() => false)) {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      if (!resp || !resp.ok()) {
        ...
        emailFailures.push(`Broken URL: Sports star page ${name} (HTTP ${resp?.status() ?? 'N/A'})`);
      } else {
  ...
  if (await videoTab.isVisible({ timeout: 4000 }).catch(() => false)) {
    ...
    for (let i = 0; i < maxVideos; i++) {
      ...
      if (!resp || !resp.ok()) {
        ...
        emailFailures.push(`Broken URL: Video page ${url} (HTTP ${resp?.status() ?? 'N/A'})`);
      } else {
  ...
  if (emailFailures.length > 0) {
    appendEmailReportFailures('DragonSports', emailFailures);
    throw new Error(`DragonSports: ${emailFailures.length} failure(s) detected`);
  }
});
