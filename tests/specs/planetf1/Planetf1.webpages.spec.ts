...
const MAX_LINKS_TO_CHECK = isSandbox ? 20 : isQuick ? 40 : 60;
const MAX_CONCURRENT_FETCH = isSandbox ? 2 : isQuick ? 4 : 6;
// Fewer deep samples to keep total runtime under budget. Headed demo: at least 1 driver/team so user sees clicks
const MAX_DRIVERS_TO_TEST = isHeadedDemo ? Math.max(1, isSandbox ? 0 : isQuick ? 1 : 2) : (isSandbox ? 0 : isQuick ? 1 : 2);
const MAX_TEAMS_TO_TEST = isHeadedDemo ? Math.max(1, isSandbox ? 0 : isQuick ? 1 : 2) : (isSandbox ? 0 : isQuick ? 1 : 2);
...
function normalizeUrl(url: string): string {
  try {
    ...
    // PlanetF1 canonical driver slug.
...
test('PlanetF1 – navigation, load, and content integrity checks', async ({ page, request }) => {
  test.setTimeout(isHeadedRun ? 10 * 60_000 : (isSandbox ? 90_000 : isQuick ? 3 * 60_000 : 10 * 60_000));
  ...
  for (const { label, url } of NAV_TABS) {
    ...
    if (/results/i.test(label)) {
      ...
      const fallbackRacePages = raceResultLinks.length === 0
        ? ['https://www.planetf1.com/results/barcelona-catalunya-grand-prix']
        : [];
      const maxRacePagesToValidate = isQuick ? 2 : 3;
      ...
      for (const raceUrl of pagesToValidate) {
        ...
        try {
          ...
          await acceptConsent();
          await page.waitForTimeout(isQuick ? 1200 : 1800);

          await clickRaceSessionTab();
          await page.waitForTimeout(isQuick ? 1500 : 2000);
          await page
            .waitForFunction(() => {
              const bodyText = document.body?.innerText || '';
              return (
                /Norris|Verstappen|Hamilton|Piastri|Russell|Leclerc|Alonso|Sainz|Gasly|Albon/i.test(bodyText) ||
    ...
    if (/data|tech/i.test(label)) {
      ...
      if (articleCount > 0) {
        ...
        // Check Sky Player autoplay (headed demo only — slow and flaky in headless CI)
        if (isHeadedDemo) {
          try {
