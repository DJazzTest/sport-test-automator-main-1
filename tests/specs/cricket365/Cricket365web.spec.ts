...
async function isCountryPage2Broken(
  ...
  // Avoid masking failures via redirects: capture true status for page2Url.
  const response = await request
    .get(page2Url, { timeout: 15_000, maxRedirects: 0 })
    .catch(() => null);
  if (!response) return { broken: true, page2Url, reason: 'no-response' };
  ...
  if (status >= 300 && status < 400) {
    // Treat redirects on page/2 as broken for pagination coverage: we expect a real page 2 list.
    return {
      broken: true,
      page2Url,
      status,
  ...
  if (status >= 400) return { broken: true, page2Url, status, resolvedUrl, reason: `http-${status}` };

  // Some endpoints return HTTP 200 but render a 404 page (soft-404). Detect via response body.
  ...
  const looksLike404 =
    ...
    lower.includes('sorry, the page') ||
    lower.includes('this page does not exist');

  if (looksLike404)
    return { broken: true, page2Url, status, resolvedUrl, redirectLocation, reason: 'soft-404' };

  return { broken: false, page2Url, status, resolvedUrl, redirectLocation };
}
...
test.describe.serial('Cricket365', () => {
  ...
  test('Full sequence: Homepage → Live Scores → Tests → ODI → T20 → Countries', async ({
    ...
    let countryPagination404: Array<{
      countryName: string;
      countryUrl: string;
      page2Url: string;
      status?: number;
    ...
    for (const section of SECTION_ORDER) {
      ...
      // Some nav pills can be animated/overlaid; ensure it's in view and use a slightly longer timeout.
      await link.scrollIntoViewIfNeeded().catch(() => {});
      try {
        await link.click({ timeout: 20_000 });
      } catch {
        // Fallback: click via DOM if Playwright thinks it's unstable.
        await link.evaluate((el: Element) => (el as HTMLElement).click()).catch(() => {});
      }
      ...
      if (section.name === 'Live Scores') {
          ...
          try {
            ...
            if (page2Check.broken) {
              countryPagination404.push({
                ...
                resolvedUrl: page2Check.resolvedUrl,
                redirectLocation: page2Check.redirectLocation,
                reason: page2Check.reason,
              });
            }
        ...
        if (countryPagination404.length > 0) {
          const lines = countryPagination404
            .slice(0, 25)
            .map(
              (c) =>
                `${c.status ?? 0}\t${c.reason ?? ''}\t${c.countryName}\t${c.page2Url}\t${c.resolvedUrl ?? ''}\t${c.redirectLocation ?? ''}`
            );
          info.attach('cricket365-country-pagination-404.txt', {
            body: `Status\tReason\tCountry\tPage2URL\tResolvedURL\tLocation\n${lines.join('\n')}`,
            contentType: 'text/plain',
          });
          console.warn(
            `⚠️  Country pagination failures (page 2):\n${lines
              .map((l) => `   - ${l}`)
              .join('\n')}${countryPagination404.length > 25 ? `\n   ... and ${countryPagination404.length - 25} more` : ''}`
          );
        }
        expect.soft(
          countryPagination404.length,
          `Country Pagination: page 2 must not return 404 (affected: ${countryPagination404.length} countries). Failing endpoints (sample): ${
            countryPagination404
              .slice(0, 5)
              .map((c) => `${c.status ?? 0} ${c.page2Url}`)
              .join('; ') || 'none'
          }`
        ).toBe(0);
      }
        ...
        sectionResults.push({
          ...
          error:
            countryPagination404.length > 0
              ? `404 on page 2 for ${countryPagination404.length} countries. Example: ${
                  countryPagination404[0]?.page2Url ?? 'n/a'
                }`
              : undefined,
        });
      }
