...
test.describe.serial('Cricket365', () => {
  ...
  test('Full sequence: Homepage → Live Scores → Tests → ODI → T20 → Countries', async ({
    ...
    for (const section of SECTION_ORDER) {
      ...
      const emailFailures = sectionResults
        .filter((s) => s.status === 'failed')
        .map((s) => `Functional: ${s.name}${s.error ? ' — ' + s.error : ''}`);
      if (emailFailures.length > 0) {
        appendEmailReportFailures('Cricket365', emailFailures);
        throw new Error(`Cricket365: ${emailFailures.length} section(s) failed`);
      }
    }
  });
});
