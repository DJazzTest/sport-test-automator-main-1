...
test("NRG – In Play events animation check (All Events)", async ({ page }) => {
  console.log(
    "🚀 Starting comprehensive NRG In Play animation detection test..."
  );

  // Navigate to NRG
  const baseUrl = "https://nrg.bet";
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    try {
      ...
      // Check for NRG animation widget using improved detection logic
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log("🎯 NRG IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY");
  ...
  console.log("\n🏆 === FINAL ASSESSMENT ===");
  if (successRate >= 80) {
    console.log("🌟 EXCELLENT: NRG has strong animation coverage");
  } else if (successRate >= 60) {
    console.log("👍 GOOD: NRG has decent animation coverage");
  } else if (successRate >= 40) {
    console.log("⚠️  MODERATE: NRG has limited animation coverage");
  } else {
    console.log("🚨 POOR: NRG has minimal animation coverage");
  }

  console.log(
    `🎖️  NRG Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(
      1
    )}%)`
  );
