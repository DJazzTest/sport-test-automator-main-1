...
test("AKBets – In Play events animation check (All Events)", async ({
  page,
}) => {
  console.log(
    "🚀 Starting comprehensive AKBets In Play animation detection test..."
  );

  // Navigate to AKBets
  const baseUrl = "https://akbets.bet";
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    try {
      ...
      // Check for AKBets animation widget using improved detection logic
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log("🎯 AKBETS IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY");
  ...
  console.log("\n🏆 === FINAL ASSESSMENT ===");
  if (successRate >= 80) {
    console.log("🌟 EXCELLENT: AKBets has strong animation coverage");
  } else if (successRate >= 60) {
    console.log("👍 GOOD: AKBets has decent animation coverage");
  } else if (successRate >= 40) {
    console.log("⚠️  MODERATE: AKBets has limited animation coverage");
  } else {
    console.log("🚨 POOR: AKBets has minimal animation coverage");
  }

  console.log(
    `🎖️  AKBets Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(
      1
    )}%)`
  );
