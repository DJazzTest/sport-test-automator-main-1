...
test("GentlemanJim – In Play events animation check (All Events)", async ({
  page,
}) => {
  console.log(
    "🚀 Starting comprehensive GentlemanJim In Play animation detection test..."
  );

  // Navigate to GentlemanJim
  const baseUrl = "https://gentlemanjim.bet";
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    try {
      ...
      // Check for GentlemanJim animation widget using improved detection logic
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log("🎯 GENTLEMANJIM IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY");
  ...
  console.log("\n🏆 === FINAL ASSESSMENT ===");
  if (successRate >= 80) {
    console.log("🌟 EXCELLENT: GentlemanJim has strong animation coverage");
  } else if (successRate >= 60) {
    console.log("👍 GOOD: GentlemanJim has decent animation coverage");
  } else if (successRate >= 40) {
    console.log("⚠️  MODERATE: GentlemanJim has limited animation coverage");
  } else {
    console.log("🚨 POOR: GentlemanJim has minimal animation coverage");
  }

  console.log(
    `🎖️  GentlemanJim Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(
      1
    )}%)`
  );
