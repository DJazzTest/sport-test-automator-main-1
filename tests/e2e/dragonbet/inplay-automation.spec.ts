...
test("DragonBet – In Play events animation check (All Events)", async ({
  page,
}) => {
  console.log(
    "🚀 Starting comprehensive DragonBet In Play animation detection test..."
  );

  // Navigate to DragonBet
  const baseUrl = "https://dragonbet.co.uk";
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    try {
      ...
      // Check for DragonBet animation widget using improved detection logic
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log("🎯 DRAGONBET IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY");
  ...
  console.log("\n🏆 === FINAL ASSESSMENT ===");
  if (successRate >= 80) {
    console.log("🌟 EXCELLENT: DragonBet has strong animation coverage");
  } else if (successRate >= 60) {
    console.log("👍 GOOD: DragonBet has decent animation coverage");
  } else if (successRate >= 40) {
    console.log("⚠️  MODERATE: DragonBet has limited animation coverage");
  } else {
    console.log("🚨 POOR: DragonBet has minimal animation coverage");
  }

  console.log(
    `🎖️  DragonBet Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(
      1
    )}%)`
  );
