...
test("PlanetF1 – In Play events animation check (All Events)", async ({
  page,
}) => {
  console.log(
    "🚀 Starting comprehensive PlanetF1 In Play animation detection test..."
  );

  // Navigate to PlanetF1
  const baseUrl = "https://www.planetf1.com";
  ...
  for (let i = 0; i < eventCount; i++) {
    ...
    try {
      ...
      // Check for PlanetF1 animation widget using improved detection logic
        ...
        failedEvents.push({
          ...
          sport: eventTitle.includes("vs")
            ? eventTitle.toLowerCase().includes("tennis")
              ...
              ? "Cricket"
              : eventTitle.toLowerCase().includes("f1")
              ? "F1"
              : "Unknown"
            : "Unknown",
        });
  ...
  console.log(`\n${"=".repeat(70)}`);
  console.log("🎯 PLANETF1 IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY");
  ...
  console.log("\n🏆 === FINAL ASSESSMENT ===");
  if (successRate >= 80) {
    console.log("🌟 EXCELLENT: PlanetF1 has strong animation coverage");
  } else if (successRate >= 60) {
    console.log("👍 GOOD: PlanetF1 has decent animation coverage");
  } else if (successRate >= 40) {
    console.log("⚠️  MODERATE: PlanetF1 has limited animation coverage");
  } else {
    console.log("🚨 POOR: PlanetF1 has minimal animation coverage");
  }

  console.log(
    `🎖️  PlanetF1 Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(
      1
    )}%)`
  );
