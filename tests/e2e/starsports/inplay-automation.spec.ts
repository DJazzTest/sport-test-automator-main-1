...
test('StarSports.bet – In Play events animation check (All Events)', async ({ page }) => {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log('🎯 STARSPORTS.BET IN PLAY COMPREHENSIVE ANIMATION TEST SUMMARY');
  ...
  console.log(`🎖️  StarSports Animation Coverage: ${passCount}/${eventCount} events (${successRate.toFixed(1)}%)`);

  recordInplayAnimationMetrics(
    test.info(),
    SITE_LABELS.starsports,
    eventCount,
    passCount,
    failCount
  );
