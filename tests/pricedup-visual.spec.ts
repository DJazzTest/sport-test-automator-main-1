...
test('Navigate to In Play, select live event, and verify event details on PricedUp', async () => {
  test.setTimeout(120_000);
  const browser = await chromium.launch({ headless: false, slowMo: 1000 });
  const page = await browser.newPage();
  try {
    // 1. Go to home page
    await page.goto('https://pricedup.bet/', { waitUntil: 'domcontentloaded' });
    await page.screenshot({ path: 'test-results/01-homepage-pricedup.png', fullPage: true });
    ...
    try {
      ...
      if (allowAllBtn) {
        ...
        await page.screenshot({ path: 'test-results/02-cookie-dismissed-pricedup.png', fullPage: true });
    ...
    for (const selector of inPlaySelectors) {
      try {
        ...
        await page.screenshot({ path: 'test-results/04-inplay-clicked-pricedup.png', fullPage: true });
    ...
    if (!foundInPlay) {
      ...
      await page.screenshot({ path: 'test-results/inplay-header-error-pricedup.png', fullPage: true });
    ...
    if (!foundAllSports) {
      ...
      await page.screenshot({ path: 'test-results/allsports-tab-error-pricedup.png', fullPage: true });
    ...
    for (let i = 0; i < toTest; i++) {
      ...
      await page.screenshot({ path: `test-results/event-detail-pricedup-${i + 1}.png`, fullPage: true });
      const widgetSelector = 'app-the-sport-widget-component .animated_widget';
      const widget = await page.$(widgetSelector);
      if (widget) {
        await page.screenshot({ path: `test-results/widget-found-event-pricedup-${i + 1}.png`, fullPage: true });
    ...
    if (widgetCount < toTest) {
      await page.screenshot({ path: 'test-results/not-enough-widgets-found-pricedup.png', fullPage: true });
