...
test('Navigate to In Play, select live event, and verify event details on BresBet', async () => {
  test.setTimeout(120_000);
  const browser = await chromium.launch({ headless: false, slowMo: 1000 });
  const page = await browser.newPage();
  try {
    // 1. Go to home page
    await page.goto('https://bresbet.com/', { waitUntil: 'domcontentloaded' });
    await page.screenshot({ path: 'test-results/01-homepage-bresbet.png', fullPage: true });
    ...
    try {
      ...
      if (allowAllBtn) {
        ...
        await page.screenshot({ path: 'test-results/02-cookie-dismissed-bresbet.png', fullPage: true });
    ...
    for (const selector of inPlaySelectors) {
      try {
        ...
        await page.screenshot({ path: 'test-results/04-inplay-clicked-bresbet.png', fullPage: true });
    ...
    if (!foundInPlay) {
      ...
      await page.screenshot({ path: 'test-results/inplay-header-error-bresbet.png', fullPage: true });
    ...
    if (!foundAllSports) {
      ...
      await page.screenshot({ path: 'test-results/allsports-tab-error-bresbet.png', fullPage: true });
    ...
    for (let i = 0; i < toTest; i++) {
      ...
      await page.screenshot({ path: `test-results/event-detail-bresbet-${i + 1}.png`, fullPage: true });
      const widgetSelector = 'app-the-sport-widget-component .animated_widget';
      const widget = await page.$(widgetSelector);
      if (widget) {
        await page.screenshot({ path: `test-results/widget-found-event-bresbet-${i + 1}.png`, fullPage: true });
    ...
    if (widgetCount < toTest) {
      await page.screenshot({ path: 'test-results/not-enough-widgets-found-bresbet.png', fullPage: true });
