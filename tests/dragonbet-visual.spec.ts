...
test('Navigate to In Play, select live event, and verify event details on DragonBet', async () => {
  ...
  try {
    ...
    await page.screenshot({ path: 'test-results/01-homepage-dragonbet.png', fullPage: true });
    ...
    try {
      ...
      if (allowAllBtn) {
        ...
        await page.screenshot({ path: 'test-results/02-cookie-dismissed-dragonbet.png', fullPage: true });
    ...
    for (const selector of inPlaySelectors) {
      try {
        ...
        await page.screenshot({ path: 'test-results/04-inplay-clicked-dragonbet.png', fullPage: true });
    ...
    if (!foundInPlay) {
      ...
      await page.screenshot({ path: 'test-results/inplay-header-error-dragonbet.png', fullPage: true });
    ...
    if (!foundAllSports) {
      ...
      await page.screenshot({ path: 'test-results/allsports-tab-error-dragonbet.png', fullPage: true });
    ...
    for (let i = 0; i < toTest; i++) {
      ...
      await page.screenshot({ path: `test-results/event-detail-dragonbet-${i + 1}.png`, fullPage: true });
      const widgetSelector = 'app-the-sport-widget-component .animated_widget';
      const widget = await page.$(widgetSelector);
      if (widget) {
        await page.screenshot({ path: `test-results/widget-found-event-dragonbet-${i + 1}.png`, fullPage: true });
    ...
    if (widgetCount < toTest) {
      await page.screenshot({ path: 'test-results/not-enough-widgets-found-dragonbet.png', fullPage: true });
