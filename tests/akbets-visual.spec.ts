...
test('Navigate to In Play, select live event, and verify event details on AK Bets', async () => {
  ...
  try {
    ...
    await page.screenshot({ path: 'test-results/01-homepage-akbets.png', fullPage: true });
    ...
    try {
      ...
      if (allowAllBtn) {
        ...
        await page.screenshot({ path: 'test-results/02-cookie-dismissed-akbets.png', fullPage: true });
    ...
    for (const selector of inPlaySelectors) {
      try {
        ...
        await page.screenshot({ path: 'test-results/04-inplay-clicked-akbets.png', fullPage: true });
    ...
    if (!foundInPlay) {
      ...
      await page.screenshot({ path: 'test-results/inplay-header-error-akbets.png', fullPage: true });
    ...
    if (!foundAllSports) {
      ...
      await page.screenshot({ path: 'test-results/allsports-tab-error-akbets.png', fullPage: true });
    ...
    for (let i = 0; i < toTest; i++) {
      ...
      await page.screenshot({ path: `test-results/event-detail-akbets-${i + 1}.png`, fullPage: true });
      const widgetSelector = 'app-the-sport-widget-component .animated_widget';
      const widget = await page.$(widgetSelector);
      if (widget) {
        await page.screenshot({ path: `test-results/widget-found-event-akbets-${i + 1}.png`, fullPage: true });
    ...
    if (widgetCount < toTest) {
      await page.screenshot({ path: 'test-results/not-enough-widgets-found-akbets.png', fullPage: true });
