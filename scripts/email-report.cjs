  ...
  if (hasFailures) {
    const classifyFailure = (failure) => {
      ...
      if (
        lower.includes('| fail:') ||
        lower.includes('fail: ') ||
        (lower.includes('animation') && lower.includes('fail')) ||
        /^(betwright|dragonbet|dragonsports|planetsports|starsports|multi-site)/i.test(failure)
      ) {
    ...
    const buildManualSteps = (label, detail) => {
      ...
      if (label === 'Animation / live widget') {
        if (url) {
          return `1) Open event URL ${url} 2) Open Live tracker if collapsed 3) Expected: live animation iframe/SVG (thesports01.com widget) 4) Actual: missing or not loading`;
        }
        return `1) Open the sport and event named in the line 2) Open Live tracker if collapsed 3) Expected: live animation iframe/SVG 4) Actual: missing or not loading`;
      }
...
async function sendEmail(subject, body) {
  ...
  // Aliases: SMTP_USER / RECIPIENTS (legacy local .env) map onto CI secret names.
  const server = (process.env.SMTP_SERVER || process.env.SMTP_HOST || 'smtp.gmail.com').trim();
  const port = (process.env.SMTP_PORT || '587').trim();
  const user = (process.env.SMTP_USERNAME || process.env.SMTP_USER || '').trim();
  const pass = (process.env.SMTP_PASSWORD || '').trim();
  const to = (process.env.ALERT_EMAIL_TO || process.env.RECIPIENTS || '').trim();
  const from = (process.env.ALERT_EMAIL_FROM || process.env.SMTP_USERNAME || process.env.SMTP_USER || '').trim();
